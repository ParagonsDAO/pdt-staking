...
describe("PDT Staking", () => {
    ...
    describe("pushBackEpoch0", () => {
        it("should NOT let non owner address to update epoch 0 end time", async () => {
            ...
            await expect(staking.connect(user).pushBackEpoch0(THIRTY_DAYS)).to.be.revertedWith(
                "NotOwner"
            );
        ...
        it("should NOT let owner epoch 0 end time if passed epoch 0", async () => {
            ...
            /// EPOCH 1 ///
            let epochBefore = await staking.currentEpoch();
            await expect(staking.connect(deployer).pushBackEpoch0(THIRTY_DAYS)).to.be.revertedWith(
                "AfterEpoch0"
            );
            let epochAfter = await staking.currentEpoch();
            expect(epochBefore.endTime).to.equal(epochAfter.endTime);
    ...
    describe("updateEpochLength", () => {
        it("should NOT let non owner address to update epoch length", async () => {
            await expect(staking.connect(user).updateEpochLength(THIRTY_DAYS)).to.be.revertedWith(
                "NotOwner"
            );
            expect(await staking.epochLength()).to.equal(ONE_DAY);
        });
    ...
    describe("transferOwnership()", () => {
        it("should NOT let non owner address transfer ownership", async () => {
            await expect(staking.connect(user).transferOwnership(user.address)).to.be.revertedWith(
                "NotOwner"
            );
            expect(await staking.owner()).to.equal(deployer.address);
        });
        ...
        it("should NOT allow ownership to be transferre to zero address", async () => {
            await expect(
                staking.connect(deployer).transferOwnership(ZERO_ADDRESS)
    ...
    describe("claim()", () => {
        it("should NOT allow claim all during epoch 0", async () => {
            /// EPOCH 0 ///
            await expect(staking.connect(user).claim(user.address)).to.be.revertedWith(
                "ClaimedUpToEpoch"
            );
        });
    ...
    describe("contractWeightAtEpoch()", () => {
        ...
        it("should fail if trying to get contract weight for future epoch", async () => {
            ...
            await expect(staking.connect(user).contractWeightAtEpoch("2")).to.be.revertedWith(
                "InvalidEpoch"
            );
        });
        ...
        it("should return proper contract weight at end of epoch", async () => {
            ...
            expect(+(await staking.contractWeightAtEpoch("1")).toString()).to.equal(
                +sumOfUserWeight1
            );
            ...
            expect(
                Number(sumOfUserWeight2 - (await staking.contractWeightAtEpoch("2")))
            ).to.be.lessThan(ONE_GWEI);
        });
    });
    ...
    describe("claimAmountForEpoch()", () => {
        it("should fail if trying to get claim amount for current epoch", async () => {
            ...
            await expect(staking.claimAmountForEpoch(user.address, "1")).to.be.revertedWith(
                "InvalidEpoch"
            );
        });
        ...
        it("should fail if trying to get claim amount for future epoch", async () => {
            ...
            await expect(
                staking.connect(user).claimAmountForEpoch(user.address, "2")
    ...
    describe("userWeightAtEpoch()", () => {
        it("should fail if trying to get user weight for current epoch", async () => {
            ...
            await expect(staking.userWeightAtEpoch(user.address, "1")).to.be.revertedWith(
                "InvalidEpoch"
            );
        });
        ...
        it("should fail if trying to get user weight for future epoch", async () => {
            ...
            await expect(
                staking.connect(user).userWeightAtEpoch(user.address, "2")
            ).to.be.revertedWith("InvalidEpoch");
        });
        ...
        it("should return proper user weight at epoch", async () => {
            ...
            expect(
                (await staking.userWeightAtEpoch(user.address, "2")) - additionalWeight
            ).to.equal(+ONE_THOUSAND);
        });
    });
    ...
    describe("contractWeight()", () => {
        ...
        it("should have weight properly when one staker", async () => {
            ...
            expect(await staking.contractWeight()).to.equal(
                await staking.userTotalWeight(user.address)
            );
        });
        ...
        it("should have weight properly with multiple stakers", async () => {
            ...
            expect(Number(sumOfUserWeight - (await staking.contractWeight()))).to.be.lessThan(
                ONE_GWEI
            );
        });
    });
